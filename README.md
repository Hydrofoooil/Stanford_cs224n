...
为了让模型真正按照预期的参数（512维）训练，并解决 PyTorch 版本兼容性问题，需要对文件做以下修改的总结：

#### 1. 修改 `run.py`

##### A. 修复“参数被写死”的 Bug

你的代码第 119-122 行强制将模型尺寸设为了 1024/768，导致命令行参数失效。
...
- 在 train 函数约第 281 行中加载模型的地方添加 weights_only=False。

  ```python
  params = torch.load(model_save_path, map_location=lambda storage, loc: storage, weights_only=False)
  ```
- 同时在 `nmt_model.py` 的 `load` 函数处（约第 519 行），同样加上 `weights_only=False`：
