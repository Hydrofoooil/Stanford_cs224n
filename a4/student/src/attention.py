...
def apply_rotary_emb(x, rope_cache):
    ...
    # Reshape x to (B, nh, T, hs/2, 2) to be able to convert to complex
    x_reshaped = x.float().reshape(B, nh, T, hs // 2, 2)
    x_complex = torch.view_as_complex(x_reshaped)

    rope = rope_cache[:T, :, :]
    # Add dimensions for batch and heads for broadcasting: (T, hs/2, 2) -> (1, 1, T, hs/2, 2)
    rope_complex = torch.view_as_complex(rope).unsqueeze(0).unsqueeze(0)

    x_rotated = x_complex * rope_complex
    rotated_x = torch.view_as_real(x_rotated).flatten(3)
