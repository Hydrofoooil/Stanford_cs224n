...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Ensure the truncation length is valid for the given document
        min_trunc = min(4, len(doc))
        max_trunc = min(max_len, len(doc))

        if min_trunc > max_trunc: # Handle very short documents
            trunc_len = max_trunc
        else:
            trunc_len = random.randint(min_trunc, max_trunc)

        # Randomly select a starting point for the truncation
        if len(doc) > trunc_len:
            #start_index = random.randint(0, len(doc) - trunc_len) 把trunc的起点也设成随机的话，发现训出来效果很不好
            start_index = 0
            truncated_doc = doc[start_index : start_index + trunc_len]
        else:
            truncated_doc = doc

        # 2. Break into three substrings
        import numpy as np
        masked_len = np.random.binomial(n=len(truncated_doc), p=0.25)
        if masked_len == 0 and len(truncated_doc) > 0:
            masked_len = 1 

        mask_start = random.randint(0, len(truncated_doc) - masked_len)
        prefix = truncated_doc[:mask_start]
        masked_content = truncated_doc[mask_start : mask_start + masked_len]
        suffix = truncated_doc[mask_start + masked_len:]
