...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        # "kept for bookkeeping purposes"(为了记账目的而保留)的意思是这个变量只是为了（比如在调试时）方便你回看原始句子长什么样，或者在需要时从中提取单词，但他不是在解析过程中被主动操作的数据结构。
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        transitions = model.predict(minibatch) # 注意 Python 的列表切片非常“宽容”，最后几次处理时batch_size大于unfinished_parses的长度了，Python 不会报错，而是会静默地截取到列表的末尾
        ...
        #注意：改变unfinished_parses以后，剩余元素和partial_parses里对应元素的映射关系不变
        #注意：len(pp.stack) == 1 不能改成 pp.stack == ["ROOT"]，因为后者是理想合法情况下的，但是模型对于最后一步的预测结果可能错误地把"ROOT"去掉留下别的单词
...
def test_step(name, transition, stack, buf, deps,
    ...
    """Tests that a single parse step returns thec expected output"""
    pp = PartialParse([])
