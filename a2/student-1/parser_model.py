...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embeddings = nn.Parameter(torch.tensor(embeddings)) #先将embeddings转换为张量，然后标记为模型的参数（这样就会参与梯度计算，成为一个可训练参数）
        ...
        nn.init.xavier_uniform_(self.embed_to_hidden_weight) #在PyTorch中末尾带 _ 的函数（如 xavier_uniform_）表示它们会“就地”修改传入的Tensor，而不会返回一个新的Tensor
    ...
    def embedding_lookup(self, w):
        ...
        w_flattened = torch.flatten(w)
        embed_selected = torch.index_select(self.embeddings, 0, w_flattened)
        x = embed_selected.view(w.shape[0], -1)
