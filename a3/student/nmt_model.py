...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.post_embed_cnn = nn.Conv1d(embed_size, embed_size, 2, padding='same') #讲义里没说是不是multi-layer的LSTM，所以默认是单层的
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int], grader_params=None) -> Tuple[
        ...
        source_padded = source_padded.to(self.device) #重要！
                                                      #在 sanity_check.py 中，测试脚本通常会在 CPU 上初始化模型，但它构造的输入数据可能被意外地放到了 GPU 上，或者你的代码中某些地方强制将数据移动到了 GPU，而模型却没有跟过去。
                                                      #在这一行显式地将 source_padded 移动到模型所在的设备。
        ...
        X_pack = pack_padded_sequence(X, source_lengths) #处理变长序列，去除序列中的填充（Padding），只让 RNN 处理真实有效的数据，从而节省计算资源并提高效率。
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X_pack) #enc_hiddens : (src_len, b, 2*h)
