...
if [ "$1" = "train" ]; then
	CUDA_VISIBLE_DEVICES=0 python -u run.py train --train-src=./zh_en_data/train.zh --train-tgt=./zh_en_data/train.en --dev-src=./zh_en_data/dev.zh --dev-tgt=./zh_en_data/dev.en --vocab=vocab.json --cuda --lr=5e-4 --patience=5 --valid-niter=200 --batch-size=128 --dropout=.3 --embed-size=1024 --hidden-size=1024
elif [ "$1" = "test" ]; then
